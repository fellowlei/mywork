...
public class LagouUtil {
    ...
    public static void dumpResult(String json,String fname) {
        try {
            FileUtils.write(new File(fname), json + "\n", true);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void multiGet(){
        ...
        for(int i=1; i<5;i++){
            ...
            try {
                String param = "first=true&pn=" + i + "&kd=java";
                String result = DefaultCookieUtil.post("https://www.lagou.com/jobs/positionAjax.json?city=%E5%8C%97%E4%BA%AC&needAddtionalResult=false", param);
                dumpResult(result,"lagou_java.txt");
                Thread.sleep(1000);
            }catch (Exception e){
                e.printStackTrace();
            }
